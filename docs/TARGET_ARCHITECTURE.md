...
- The first product slice uses one Tandem-scoped `memories` entity with strict category metadata, participant/tag joins, PostgreSQL full-text search, optimistic versions, and append-only activity records. New memory writes do not use Firebase; provider enrichment, media, nostalgia anniversaries, Redis features, and realtime updates remain later milestones.
